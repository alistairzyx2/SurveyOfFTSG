# Point-by-Point Response to Technical Check and Peer Review

**Manuscript ID:** 5a3d24b0-5398-4e89-835d-34efbe8bd9e3 v2.0
**Title:** Financial Time-Series Generation: A Systematic Survey of Taxonomy, Evaluation, Datasets, and Open Challenges
**Authors:** Yingxiao Zhang, Jiaxin Duan, Yue Zou, Ke Feng, Jing Xiao
**Journal:** *npj Artificial Intelligence*

Dear Dr. Monika Samra and Editorial Team of *npj Artificial Intelligence*,

We thank the editorial team for the careful technical check of our manuscript and for the constructive comments of the two reviewers during the peer-review round. This document contains our point-by-point responses to both rounds:

- **Part I — Response to the Technical Check** (16 points raised in the Technical Check letter);
- **Part II — Response to Reviewer 1 and Reviewer 2** (peer-review comments).

For the technical check, each editor's comment is stated first in full, followed by our corresponding response, as required. The manuscript has been comprehensively restructured into the required **IRDM** format, unnumbered section headings have been adopted, the abstract has been condensed to a single paragraph of 179 words, all declarations have been fully integrated into the manuscript, references have been converted to the sequential Nature numbering style (130 entries), figure legends have been thoroughly enhanced, clean and marked-up versions have been prepared, and a completed PRISMA 2020 checklist has been provided as Supplementary Information.

---

# Part I — Point-by-Point Response to Technical Check

## Point 1: Author Affiliations and Formatting
...
1. $^{1}$ School of Software and Microelectronics, Peking University, Beijing, China.
2. $^{2}$ Pingan Bank Co., Ltd., Shenzhen, China.
3. $^{3}$ China Electronics Cloud Technology Co., Ltd., Beijing, China.
4. $^{4}$ Pingan Group Co., Ltd., Shenzhen, China.
...
**Response.** We have restructured and condensed the abstract into a **single cohesive paragraph of exactly 179 words** without any subheadings, bold headers, or paragraph breaks. The revised abstract succinctly synthesizes the motivation, two-level taxonomy, evaluation protocols, benchmark catalogs, and open challenges. We confirm that this identical 179-word text has also been synchronized in the online SNAPP submission system.
...
**Response.** We have configured `\setcounter{secnumdepth}{0}` in the LaTeX preamble to remove all numerical identifiers from section, subsection, and subsubsection headings across the entire manuscript. Furthermore, we conducted a rigorous full-text audit and replaced all textual and parenthetical section number references (e.g., "Section 2", "($\S$3)", "Sections 3--6", "$\S$8") with their corresponding explicit section titles (e.g., "the Problem Formulation and Taxonomy subsection", "the Evaluation Protocols subsection", "the Applications subsection", "the Methods section", etc.).
...
**Response.** All four previous subheadings in the Introduction ("Background and Motivation", "Why Existing Surveys are Insufficient", "Scope and Contributions", and "Organization of the Paper") have been removed. The Introduction now progresses smoothly through continuous, coherent paragraphs. The final paragraph has also been updated to explicitly outline the IRDM workflow of the manuscript.

---

## Point 5: IRDM Structural Organization

> *"Format of Research Article: IRDM. Please arrange your manuscript according to the below order: Abstract (up to 200 words, without subheadings), Introduction (no subheadings permitted), Results (subheadings are compulsory), Discussion (no subheadings permitted), Limitations/Conclusions (not permitted; incorporate into Discussion), Methods (subheadings are compulsory), Declaration statements: (1) Data Availability; (2) Code Availability (where applicable); (3) Acknowledgements (including Funding); (4) Author Contributions; (5) Competing Interests. References list, Figures, Tables and associated legends, Supplementary information, Mandated checklist."*

**Response.** The entire manuscript has been reorganized to strictly adhere to the IRDM structure:

- **Abstract:** Single paragraph, 179 words, unnumbered, no subheadings.
- **Introduction:** Unstructured, no subheadings.
- **Results:** Features compulsory thematic subheadings: (1) Problem Formulation and A Two-Level Taxonomy; (2) Financial Time-Series Extrapolation; (3) Financial Time-Series Imputation; (4) Financial Time-Series Synthesis; (5) Applications; (6) Evaluation Protocols; (7) Data Resources and Benchmarks.
- **Discussion:** Contains no subheadings (per editorial guidelines); organizes open challenges, limitations, and concluding outlook via bold paragraph lead-ins.
- **Methods:** Positioned directly after Discussion with compulsory subheadings: (1) Literature Retrieval Strategy; (2) Inclusion and Exclusion Criteria; (3) Data Extraction and Coding Protocol; (4) Review Dimensions and Taxonomy Synthesis.
- **Declarations:** Follows Methods and precedes References.
- **References:** Nature-compliant sequential numbered bibliography.

---

## Point 6 & 7: Renaming Survey Sections and Incorporating Conclusions
...
**Response.** In accordance with this directive:

1. The previous "Methodology of the Survey" section has been renamed to "Methods" and placed after Discussion.
2. The taxonomic and synthesis bodies (Problem Formulation, Extrapolation, Imputation, Synthesis, Applications, Evaluation Protocols, Data Resources) have been nested under the "Results" section with compulsory subheadings.
3. The "Open Challenges and Future Directions" content and the former "Conclusions" section have been fully integrated into the unified "Discussion" section under the thematic lead-in "Limitations, practical implications, and concluding outlook", without any subheadings.
...
**Response.** We have included the exact required sentence under the mandatory heading in the manuscript:
...
## Point 10: Reference List in Nature Format

> *"Please ensure the references are in the standard Nature format. References should be numbered sequentially as they appear in the text, methods, tables, and figure legends using a 1, 2, 3, format. They should follow the sequence: author list, title of the paper, name of the journal, volume number, initial-final page numbers or article number (year)... bibliography software has been used, please check that the reference list and citations have not been corrupted or display errors. Please either provide a separate BIB/BIBL file, or recompile your LaTeX file to include this information."*

**Response.** We have fully migrated the citation and bibliography pipeline to the official Nature sequential numerical format using `naturemag.bst`. All hidden pre-citation macros have been eliminated, ensuring that references are numbered strictly in the order of their first appearance in the visible text: superscript 1 (Zhao et al., 2025) and superscript 2 (Jiang et al., 2025) on page 1 of the Introduction, followed sequentially by 3, 4, 5... through 130. The reference list entries appear as 1., 2., 3.... 130. in Nature's sequence format (author list, paper title, journal name in italics, volume number in bold, initial--final page numbers, and publication year). The bibliography file (`ultimate_complete.bib`) is provided separately, and the manuscript has been recompiled so that all citations and the reference list render correctly with no corruption or display errors.

---

## Point 11: Individual Figure Files, Relocation after References, and Extended Legends

> *"Please provide figure files individually in the system (Figure 1, Figure 2, Figure 3...etc.)... Add the legends in the main manuscript file after references. Figure titles and legends must be removed from the image file... Please remove the figure label (e.g. Figure 1) from the image. Begin each figure legend with a concise sentence summarizing the content of the figure; this will serve as the figure title. This should be followed by a detailed description of the figure in the following sentences, up to 350 words, to serve as the legend body. Define all abbreviations, symbols, and color codes used in the figure. Do not use graphical symbols directly, describe them in words (e.g., blue circles, red dashed line). Present multi-panel figures on a single page, labeled a), b), c), etc., with each panel described individually. This requirement also applies to all supplementary figures."*

**Response.** All requirements regarding figures, tables, and legends have been systematically addressed:

1. **Repositioned after References:** Per the journal's IRDM layout instructions, all figure environments and tables have been moved from the main body and placed into dedicated sections after the References list (Figure Legends and Figures and Tables).
2. **Figure Numbering and Callout Order:** All figure callouts in the text now appear in strict chronological order from Figure 1 through Figure 6.
3. **Extended Legends:** Each figure caption begins with a concise bold summary sentence serving as the title (e.g., **Figure 1 | Evolution of Financial Time-Series Generation techniques over the past decade.**), followed by a comprehensive description (under 350 words) defining all subpanels (**a**, **b**, **c**), line styles, and color codes in plain English words without raw graphical symbols. All acronyms (e.g., NMTucker, RegTensor, NDDP, CSDI, MarS) are explicitly defined.
4. **Clean Standalone Figure Files:** High-resolution standalone figure files (`Figure1.pdf`, `Figure2.pdf`, `Figure3.png`, `Figure4.pdf`, `Figure5.pdf`, and `Figure6.png`) have been exported into a dedicated folder (`separate_figures/`) completely stripped of embedded figure labels (e.g., "Figure 1") and titles, formatted for individual upload to the submission system.
...
**Response.** We have prepared both required PDF documents:

- **Clean version (`is26.pdf`):** A completely clean, unannotated manuscript in strict IRDM format with Nature numbered references, unnumbered headings, and post-reference float layout, ready for direct production typesetting.
- **Marked-up version (`is26_marked.pdf`):** Features a prominent revision banner and a dedicated summary block detailing all technical modifications across affiliations, abstract condensation, IRDM restructuring, declaration integration, citation re-indexing, and float repositioning, submitted as a Related manuscript file.

---

## Point 13: Data Availability and Code Availability Declarations
...
## Point 14: Figure Copyright and Originality Confirmation in Cover Letter
...
**Response.** To prevent ambiguity in the submission system and author registry where two co-authors share identical initials (Yingxiao Zhang and Yue Zou), we have explicitly disambiguated their citations in the Author Contributions section:

> **Author Contributions:** "Y. Zhang: Conceptualization, Methodology, Writing -- Original Draft. J. Duan: Validation, Formal analysis, Writing -- Review & Editing. Y. Zou: Validation, Formal analysis, Writing -- Review & Editing. K. Feng: Supervision, Funding acquisition, Project administration. J. Xiao: Supervision, Funding acquisition, Project administration. All authors reviewed, discussed, and approved the final submitted version of the manuscript."

This updated statement has been matched verbatim in the online SNAPP portal.

---

## Point 16: Mandated Systematic Review Checklist (PRISMA 2020)
...
**Response.** We have prepared a dedicated Supplementary Information document (`Supplementary_Information_PRISMA.pdf`) containing the complete, itemized **PRISMA 2020 Checklist** across all 27 standard reporting items, detailing the exact manuscript location and page number where each item is addressed. In addition, all declarations (Data Availability, Code Availability, Acknowledgements, Funding, Author Contributions, Competing Interests) appear in the main manuscript file rather than in the supplementary file, in accordance with the final sentence of the technical check letter.

---

We believe these comprehensive revisions fully satisfy all editorial requirements of *npj Artificial Intelligence*. Thank you again for your valuable editorial guidance.

---

# Part II — Response to Peer Review (Reviewers 1 and 2)

The following section reproduces our responses to the peer-review comments of Reviewer 1 and Reviewer 2. Page and section references therein correspond to the revised manuscript as submitted for the second version (v2.0) after the technical check.

> **Note on section references.** The `§X.Y` references in Part II (e.g., `§6.2`, `§7.3`) follow the numbered section scheme of the version the reviewers saw. As detailed in Part I, Point 3 and Point 6–7, the manuscript has since been restructured into the IRDM format with unnumbered headings: the old numbered sections now correspond to the Results subsections (e.g., old `§6` ≈ Results “Financial Time-Series Synthesis”) and the Methods/Discussion sections. All substantive content described in Part II has been carried over into the current version unchanged.
...
1. **Terminology is now unambiguous.** FTSG is reserved exclusively for the umbrella domain; the three sub-tasks are FTSE, FTSI, and FTSS. A definition paragraph has been added to the Problem Formulation subsection.
...
3. **The screening arithmetic is now internally consistent.** The corpus of 170 studies is stated identically across the manuscript, and the approximate "2500+"/"600+" labels in Figure 2 have been replaced with exact counts that reconcile with the retrieval statistics table.
4. **A new comparison table positions our survey against the six prior surveys** along the five dimensions Reviewer 2 requested.
5. **The extrapolation, imputation, and synthesis sections have been substantially rewritten** with mechanism-level analysis, the numbers the primary sources actually report, and explicit critical commentary. TAIL-GAN now receives a dedicated paragraph on the joint elicitability of VaR and ES.
6. **Unsupported claims have been removed or grounded.** The "18% of months" figure and the unreferenced US Treasury / Bank of England statement are gone. The "vastly outperform" assertion no longer appears as a claim of ours; the phrase survives in the Applications section only inside quotation marks, where we identify that reading as *unsupported* by the current evidence.
