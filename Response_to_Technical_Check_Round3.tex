...
\vspace{0.5em}
\noindent\textbf{(i) Secondary subheadings removed.} In the previous revision, the seven compulsory primary subheadings of the Results section were themselves typeset as lower-level headings, and a further 38 headings sat beneath them. All 38 of these secondary subheadings have now been deleted, and the seven primary subheadings have been raised to the primary level the journal requires, so that no heading now exists below the primary level anywhere in Results. One further heading was also removed: an empty ``Summarization'' heading at the end of the Synthesis section that carried no text of its own. A red bracketed marker is placed at each of these 38 positions in the marked-up manuscript---37 of the form \textcolor{red}{[Secondary subheading ``\ldots'' removed.]} and one recording the deletion of the empty trailing heading---so that each deletion can be checked individually against this letter. The primary subheadings that remain in Results are exactly the following seven, all of which are the compulsory IRDM subheadings requested by the journal:
...
\vspace{0.5em}
\noindent\textbf{Changes shown in red font.} Every change made in this revision is rendered directly in red font, so that it is visible in any standard PDF reader without any markup tooling. A total of 111 red-font sites now appear across 28 pages of the manuscript. All newly written prose (the former list content of Results, the former list content of Methods, and the reworded sentences) is printed in red. In addition, red bracketed markers are placed at each of the 38 positions where a heading was deleted---37 secondary subheadings of the Results section plus the one empty trailing heading described above---and 9 further markers record the points at which an itemized or enumerated list was converted into running prose, so that the editor can verify the restructuring point by point and confirm that no secondary heading has been overlooked.
