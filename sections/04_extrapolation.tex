...
\revnote{Secondary subheading ``Problem Characteristics'' removed; its content now continues directly beneath the primary heading.}
...
\revnote{The numbered list of problem characteristics below was converted into running prose, as numbered text under a primary heading is treated as a secondary subheading.}
The problem exhibits several distinct characteristics that differentiate it from general sequence extrapolation. \revadd{First, generation must be strictly causal: future values depend solely on past observations without any look-ahead bias, which is analogous to autoregressive language modelling but with continuous, real-valued tokens. Second, financial time series are non-stationary, exhibiting time-varying statistical properties such as changing volatility regimes and structural breaks; the conditional distribution $p(\mathbf{x}_{t+1:T} | \mathbf{x}_{1:t})$ itself evolves over time, requiring models to adapt to distributional shifts. Third, market data are characterised by a low signal-to-noise ratio, so separating genuine predictive signals from random fluctuations is difficult and deterministic models tend to overfit. Fourth, high-dimensional dependencies arise in multivariate settings, where assets exhibit complex cross-sectional relationships such as lead--lag effects and sector correlations that must be modelled for coherent joint generation.}
...
\revnote{Secondary subheading ``Classical and Hybrid Predictive Models'' removed.}
...
\revnote{Secondary subheading ``Generative Models for Extrapolation'' removed.}
...
\revnote{Secondary subheading ``Time Series Foundation Models'' removed.}
...
\revnote{Secondary subheading ``Emerging Trends'' removed.}
...
\revnote{Secondary subheading ``Summarization'' removed.}
