\subsection{Applications\revadddec{: Extrapolation and the Application Landscape}}
...
the observation pipeline for PPO/A2C/SAC trading agents\revadddec{~\citebox{\citep{schulman2017proximal,mnih2016asynchronous,haarnoja2018soft}} }trained on
...
achieve higher Sharpe \revadddec{ratios~\citep{sharpe1994sharpe} }and lower maximum drawdown than agents trained on
...
allocation. \revdeldec{ More recently, LLM-driven forecasters
(}Time-LLM~\citep{jin2024timellm} \revdeldec{ , }\revadddec{and }CALF~\citep{liu2025calf} \revdeldec{ ) have been
evaluated as the predictor component inside FinRL-style pipelines, with consistent gains on A-share and crypto benchmarks}\revadddec{are representative financial forecasters with adapter architectures related to these workflows; direct, standardized FinRL comparisons remain limited}.
...
to \revdeldec{ demonstrate regulatory-grade coverage }\revadddec{examine coverage under VaR/ES back-testing criteria~\citep{acerbi2014expected}}. On the option-pricing side,
deep generative models of the underlying (e.g., quant GAN variants) have
been used to produce Monte-Carlo paths\revadddec{~\citep{boyle1977options} }that replace historical
...
the Markowitz risk-aversion level\revadddec{~\citep{markowitz1952portfolio}}, and the resulting synthetic future
paths are scored by an optimizer that selects weights without committing
to a single forecast.

% --- 5.3.2 Applications of FTSI -------------------------------------
\subsection{Applications\revdeldec{ of FTSI}\revadddec{: Imputation}}
...
agent. \revdeldec{ GAIN}\revadddec{Yoon et al.}~\citep{yoon2018gain} introduced \revadddec{GAIN, }the generative-adversarial
imputation paradigm\revdeldec{ and }\revadddec{, and it }remains a baseline in many open finance
...
smaller distributional drift on the K-line dataset than linear \revadddec{interpolation }or MICE
 baselines\revadddec{~\citep{vanbuuren1999multiple}}. Beyond price reconstruction, missing-data scenarios in
...
performance on financial tabular data. For instance, \revdeldec{ evaluations of
Transformer-enhanced GAN oversampling}\revadddec{Emaan}~\citep{emaan2025improving} \revdeldec{ report
that }\revadddec{reports
that Transformer-enhanced GAN oversampling for }synthesizing minority fraud transactions yields substantial gains
in detection recall compared to heuristic \revdeldec{ interpolation baselines }\revadddec{oversampling methods }such as
SMOTE\revadddec{~\citep{chawla2002smote} }on standard credit-card transaction benchmarks;
...
\subsection{Applications\revdeldec{ of FTSS}\revadddec{: Synthesis}}
...
OpenAI Gym interface\revadddec{~\citep{brockman2016openai}}, enabling head-to-head backtests of RL agents that
interact with thousands of background traders.
\revadddec{Balch et al.~}\citep{balch2019how} compare single-agent market replay against
...
strategy rankings. \revadddec{Rollins and Cliff~}\citep{rollins2020which} extend this with a threaded
...
manufacture counterfactual \revdeldec{ or Black-Swan }\revadddec{tail }scenarios that historical
...
that the synthesized tails \revdeldec{ pass the Basel }\revadddec{perform well in VaR/ES }back-tests where naive
\revdeldec{ block-bootstrap fails}\revadddec{block bootstrap~\citep{politis1994stationary} performs less reliably}. Hybrid CGAN for Portfolio
Analysis~\citep{lu2022hybrid} generates market scenarios conditioned on
risk aversion, allowing a portfolio engine \revadddec{grounded in mean--variance optimization~\citep{markowitz1952portfolio} }to be re-evaluated against
...
scenarios, and the authors show that downstream mean-\revadddec{-}variance and
risk-parity allocations\revadddec{~\citep{maillard2010properties} }computed on the synthetic samples exhibit
...
scenarios for risk models that are \revdeldec{ notoriously }sensitive to data
artifacts.
...
on metrics such as realized spread and adverse-selection cost\revadddec{~\citebox{\citep{glosten1985bid,huang1996dealer}} }that are invisible to bar-level environments. Robust Risk-Sensitive RL
...
\revadddec{In privacy-preserving data sharing and regulatory scenarios, }synthetic financial data is a recognized route to overcome the friction
between data sharing and the privacy constraints imposed by GDPR, CCPA,
and the equivalent Chinese PIPL. \revadddec{Balch et al.~\citep{balch2024six} propose the }Six Levels of
Privacy \revdeldec{ ~
proposes a six-tier framework and demonstrates }\revadddec{framework and demonstrate }empirically that synthetic equities, options and LOB
...
counts) while reducing \revdeldec{ membership-inference vulnerability to }\revadddec{vulnerability to membership-inference attacks~\citep{shokri2017membership} to
}near-baseline. \revdeldec{ Privacy Measurement in Tabular Synthetic
Data}\revadddec{Boudewijn et al.}~\citep{boudewijn2023privacy} formalize\revdeldec{ s} the corresponding
evaluation protocol that quantifies the privacy/utility frontier. On
the regulatory side, \revadddec{Balch et al.~}\citep{balch2024six} \revadddec{further }argue that FTSS pipelines can
...
% --- 5.3.4 Cross-cutting ---------------------------------------------
\subsection{Cross-cutting \revdeldec{ Use}\revadddec{Applications}: Synthetic Datasets as Benchmarks}
